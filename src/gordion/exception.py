...
class UpdateError(Exception):
  """Exception raised for user facing errors that an operation cannot be performed."""
  def __init__(self, repo_path, reason):
    self.message = f"Cannot update {repo_path} because {reason}"
    super().__init__(self.message)

class ActiveBranchAheadError(UpdateError):
  """Exception raised for user facing errors that an operation cannot be performed."""
  def __init__(self, repo_path, active_branch, remote_branch, num_commits):
    ...
    super().__init__(repo_path, reason)
