...
class RepositoryFolder(Folder):
  ...
  def is_other_branch(self) -> bool:
    if not self.repo.handle.head.is_detached:
      return not self.is_default_branch() and not self.is_root_branch()
    return False

  def does_root_branch_have_commit(self):
  ...
  def does_default_branch_have_commit(self):
  ...
  def color_branch(self, branch_name: str):
    ...
    elif self.is_default_branch():
      if self.does_root_branch_have_commit():
        return gordion.utils.yellow(branch_name)
      else:
        return gordion.utils.green(branch_name)

    # Case3: Other branch is checked out.
    elif self.is_other_branch():
      return gordion.utils.yellow(branch_name)
    ...
    # Case4: DETATCHED
      ...
      if self.does_root_branch_have_commit():
        return gordion.utils.yellow(branch_name)
      elif self.does_default_branch_have_commit():
        return gordion.utils.yellow(branch_name)
      else:
        return gordion.utils.green(branch_name)

  def get_branch_suggestion(self):
  ...
  def is_detached_head_saved(self) -> bool:
  ...
  def get_branch_warnings(self, branch_suggestion: Optional[str]):
    # Generate warnings
    ...
    if not self.repo.handle.head.is_detached:
      ...
      if self.is_tracked():
        ...
        if self.is_correct_tracking_branch():
          pass
        else:
    ...
    # Detached
      ...
      if not self.is_detached_head_saved():
  ...
  def is_correct_tracking_branch(self) -> bool:
  ...
  def get_display_name(self) -> str:
    ...
    # Branch header.
    branch_header = self.get_branch_name()
    branch_suggestion = None
    if is_repository_listed:
      branch_header = self.color_branch(branch_header)
      branch_suggestion = self.get_branch_suggestion()

    branch_header += self.get_branch_warnings(branch_suggestion)
